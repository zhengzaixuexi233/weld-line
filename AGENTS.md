...
- **自动扫描**：选择"图像文件"源时自动扫描 `data/images` 目录并加载图片，图片源会自动执行一次检测

### 3. GUI模块 (src/gui/)

- **MainWindow**：主窗口，显示图像和检测结果
- **ControlPanel**：控制面板，参数调整
- **功能**：摄像头预览、图片自动检测、图像/视频自动加载、拖放支持、恢复默认参数、放大图像滚轮缩放（0.8-3倍）与左键拖拽视角

### 4. 配置管理 (src/config/)

- **ConfigManager**：YAML配置文件管理
- **参数模板**：支持创建、保存、删除、切换参数模板，本地持久化存储
...
- `angle_tolerance`：角度容差（默认15度，GUI范围1-45度）
...
1. 选择输入源（摄像头/视频/图像）
2. 调整检测参数（可选）
3. 摄像头/视频点击"开始检测"；图片源会在选择或拖入后自动检测
4. 查看实时检测结果
