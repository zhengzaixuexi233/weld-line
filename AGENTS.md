...
当前版本已完成完整的GUI功能：实时检测、参数模板管理、图像放大查看、自动保存录像/截图、多摄像头切换、视频进度条拖拽跳转等。
...
- **MainWindow**：主窗口
  ...
  - 拖放文件加载（图片/视频）
  - 放大查看窗口（点击任意图片弹出）
    - 滚轮缩放（1.0×–3.0×，步长 0.2），实时刷新时保持当前缩放与平移视角
    - 鼠标左键拖拽平移视角
    - 系统标题栏最大化按钮
    - 摄像头/视频源实时更新画面
  ...
  - 状态栏显示检测数量和当前状态
    - 视频检测完毕后可重新开始（seek(0)）
    - 视频进度条：显示区底部，支持点击跳转/拖拽跳转，自动暂停/恢复播放，时间标签宽度随内容自适应
    - 放大窗口进度条：放大查看视频时底部显示，播放/暂停按钮（▶/⏸），拖拽跳转，与主进度条同步，时间标签宽度随内容自适应
    - 全局空格键快捷键：随时切换检测状态（不依赖焦点）
- **ControlPanel**：控制面板（建议宽度 400px，允许在宽窗口下自适应扩展）
  - 输入源组：输入源选择 + 多摄像头切换（仅摄像头源可见）
  - 检测参数组（合并预处理+检测）：模板管理（置顶，默认单行显示，仅在宽度不足时换为两行）→ 所有参数滑块 → 恢复默认（置底）
  - 显示选项组：独立开关三图显示
  - 检测按钮行：开始检测（拉伸占满）→ 自动保存复选框 → 打开保存文件夹按钮
  - 所有参数支持悬停提示（含作用、原理、调整建议）
  - 参数模板管理：创建/保存/删除/切换，combo 可编辑名称，日志记录每次操作
  - CLAHE 限制、Canny 阈值等浮点参数支持滑块拖拽 + 数值输入，右侧数值输入框按当前参数范围统一宽度对齐，避免额外临时控件泄漏到界面
...
6. **布局约束**：控制面板最小宽度 400px、最大宽度 480px，主窗口最小宽度 1240px；主布局优先保证图像区宽度，参数标签宽度基于当前字体动态计算，模板区默认单行、空间不足时自动换行，参数数值框宽度按项目实际取值范围统一计算，在最小宽度下仍保留可见滑条区域
